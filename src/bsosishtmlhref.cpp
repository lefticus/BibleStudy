...
BSOSISHTMLHREF::MyUserData::MyUserData(const SWModule *module, const SWKey *key) : BasicFilterUserData(module, key) {
	if (module) {
...
bool BSOSISHTMLHREF::handleToken(SWBuf &buf, const char *token, BasicFilterUserData *userData) {
				...
				if (attrib = tag.getAttribute("lemma")) {
					...
					do {
						...
						if ((!strcmp(val2, "3588")) && (lastText.length() < 1))
							...
							if (!u->suspendTextPassThru)
								buf.appendFormatted(" <small><em>&lt;<a href=\"module://passagestudy.jsp?action=showStrongs&type=%s&value=%s\">%s</a>&gt;</em></small> ", 
										(gh.length()) ? gh.c_str() : "", 
					...
					if (show) {
						...
						do {
							...
							if ((*val == 'T') && (strchr("GH", val[1])) && (isdigit(val[2])))
								val2+=2;
							if (!u->suspendTextPassThru)
								buf.appendFormatted(" <small><em>(<a href=\"module://passagestudy.jsp?action=showMorph&type=%s&value=%s\">%s</a>)</em></small> ", 
				...
				if (!tag.isEmpty()) {
					...
					if (type != "strongsMarkup") {	// leave strong's markup notes out, in the future we'll probably have different option filters to turn different note types on or off
						...
						if (vkey) {
							...
							buf.appendFormatted("<a href=\"module://passagestudy.jsp?action=showNote&type=%c&module=%s&passage=%s.n.%s\"><small><sup>*%c</sup></small></a> ", 
								ch, 
